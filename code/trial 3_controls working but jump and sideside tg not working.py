...
def press(keycode, modifier=0):
    kb._send(__import__('struct').pack("8B", modifier, 0, keycode, 0, 0, 0, 0, 0))
...
while True:
    if kb.is_connected():
        ...
        try:
            ...
            # Find which key to send (priority: Fireboy first, then Watergirl)
            # BLE HID can technically hold multiple but this keeps it simple
            active_fb = next((k for k in ['fb_l','fb_r','fb_j'] if touches[k]), None)
            active_wg = next((k for k in ['wg_l','wg_r','wg_j'] if touches[k]), None)
            ...
            # Build and send a report with both players' keys simultaneously
            import struct
            k1 = keys[active_fb] if active_fb else 0
            k2 = keys[active_wg] if active_wg else 0
            kb._send(struct.pack("8B", 0, 0, k1, k2, 0, 0, 0, 0))
